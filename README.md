...
# Step 4
- I used ArcPro for the final task as well. After downloading the `NTA Shapefile`, I imported it into a new `GDB`. Next I used a definition query to filter the `raw.csv` file for `HEAT/HOT WATE`' complaint types only, then I exported it into the geodatabase as a `GDB` file.
